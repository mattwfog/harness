...
*"fix total rounding"*) k=$(n c2)
  ...
  else echo "The harness rejected the edit to tests/test_money.py (outside the owned path). Restoring the test and fixing the implementation instead."; printf 'from app.money import total\n\ndef test_total_exact():\n    assert total([1, 2, 3]) == 6\n' > tests/test_money.py; printf 'def total(items):\n    return sum(items)\n' > app/money.py; echo "Removed the stray multiplier in total(); the original exact assertion passes."; fi ;;
esac
